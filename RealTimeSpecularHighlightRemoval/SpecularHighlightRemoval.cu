...
void SpecularHighlightRemoval::initialize(int imageRows, int imageCols)
{
	...
	deviceMinimumImage = cv::cuda::GpuMat(imageRows, imageCols, CV_8UC1);
	deviceMaximumImage = cv::cuda::GpuMat(imageRows, imageCols, CV_8UC1);
	deviceRangeImage = cv::cuda::GpuMat(imageRows, imageCols, CV_8UC1);
	deviceMaskImage = cv::cuda::GpuMat(imageRows, imageCols, CV_8UC1);
	deviceMinimumChromaticityImage = cv::cuda::GpuMat(imageRows, imageCols, CV_32FC1);
	deviceMaximumChromaticityImage = cv::cuda::GpuMat(imageRows, imageCols, CV_32FC1);
...
cv::Mat SpecularHighlightRemoval::run(cv::Mat image)
{
...
#ifdef REMOVE_SPECULAR_HIGHLIGHT_USING_CUDA
	deviceOriginalImage = cv::cuda::GpuMat(image);
    ...
    cv::Mat hostMinimumImage;
    deviceMinimumImage.download(hostMinimumImage);
//    cv::meanStdDev(deviceMinimumImage, minimumMean, stdDevMean);
    cv::meanStdDev(hostMinimumImage, minimumMean, stdDevMean);
    ...
    deviceMinimumChromaticityImage.download(hostMinimumImage);
//    cv::minMaxLoc(deviceMinimumChromaticityImage, &minimumValue, &maximumMinimumValue, &minimumLocation, &maximumMinimumLocation, deviceMaskImage);
    cv::minMaxLoc(hostMinimumImage, &minimumValue, &maximumMinimumValue, &minimumLocation, &maximumMinimumLocation, hostMaskImage);
    std::cout << "RETURN2\n";

//    deviceMinimumChromaticityImage.download(hostMinimumImage);
    cv::minMaxLoc(hostMinimumImage, 0, &maximumValue, 0, &maximumLocation, hostMaskImage);
//    cv::minMaxLoc(deviceMaximumChromaticityImage, 0, &maximumValue, 0, &maximumLocation, deviceMaskImage);
    std::cout << "RETURN3\n";
